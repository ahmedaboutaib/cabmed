...
import { createTypeBilan, deleteTypeBilan } from "../../../../utils/apis3";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faTrash } from "@fortawesome/free-solid-svg-icons";

const AjouterTypeBilan = ({
  changeFormType,
  formType,
  remplirType,
  typee,
  ...
  const sbmitType = (event) => {
    ...
    const createType = async () => {
      ...
      try {
        ...
        if (type?.titre) {
          const response = await createTypeBilan({
            ...type,
            CabinetId: CabinetId,
          });
  ...
  const deleteTypee = async (id) => {
    try {
      ...
      const response = await deleteTypeBilan(id);
      remplirType();
    } catch (error) {
...
export default AjouterTypeBilan;
